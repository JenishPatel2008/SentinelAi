...
import { API_BASE_URL, acknowledgeAlarm, createCamera, createTrustedPerson, createWatchlistEntry, decideAlert, deleteCamera, deleteTrustedPerson, deleteWatchlistEntry, getAlarm, getAlert, getAlerts, getAnalytics, getCameras, getCamerasWithStatuses, getCurrentUser, getDetections, getEvents, getHealth, getIncidents, getPlateHistory, getSettings, getStreamPreviewUrl, getStreamStatus, getTrustedPersons, getWatchlist, loginOperator, logoutOperator, replaceTrustedPersonReference, silenceAlarm, startStream, stopStream, testRtspConnection, updateCamera, updateIncident, updateSettings, updateTrustedPerson, updateWatchlistEntry, uploadVideo } from "./services/api";
import "./App.css";
import ZoneEditor from "./components/zones/ZoneEditor";
import useWebSocket from "./hooks/useWebSocket";

const nav = [["Dashboard", "/dashboard", LayoutDashboard], ["Live Monitoring", "/monitoring", Video], ["Alerts", "/alerts", Bell], ["Analytics", "/analytics", BarChart3], ["Archive", "/events", Activity], ["Plate History", "/plate-history", Search], ["Plate Watchlist", "/watchlist", Shield], ["Cameras", "/cameras", Camera], ["Border Map", "/border-map", Map], ["Trusted Persons", "/trusted-persons", Shield], ["Incidents", "/incidents", AlertTriangle], ["Settings", "/settings", Settings]];
...
function useRemote(loader, initial, normalize, refreshInterval = 0) {
  const [data, setData] = useState(initial); const [loading, setLoading] = useState(true); const [error, setError] = useState(null); const [version, setVersion] = useState(0); const silentRefresh = useRef(false);
  useEffect(() => { let mounted = true; if (!silentRefresh.current) setLoading(true); silentRefresh.current = false; loader().then(normalize).then((value) => { if (mounted) setData(value); }).catch((err) => { if (mounted) setError(err.message || "Unable to load surveillance data."); }).finally(() => { if (mounted) setLoading(false); }); return () => { mounted = false; }; }, [loader, normalize, version]);
  const reload = (silent = false) => { silentRefresh.current = silent; setError(null); setVersion((current) => current + 1); };
  const pollingInterval = refreshInterval || (loader === loadCameraRecords ? 3000 : 0);
  useEffect(() => { if (!pollingInterval) return undefined; const timer = setInterval(() => { silentRefresh.current = true; setError(null); setVersion((current) => current + 1); }, pollingInterval); return () => clearInterval(timer); }, [pollingInterval]);
...
function LivePreview({ camera, alt }) {
  ...
  if (!camera.is_active || !camera.stream_url || !["starting", "connecting", "online"].includes(camera.status)) return <div className="camera-offline">{camera.status === "unavailable" ? "STREAM STATUS UNAVAILABLE" : "OFFLINE / NO SOURCE"}</div>;
...
function AlertNotification() {
  ...
  useEffect(() => {
    if (!message?.data || !["alert", "intrusion_alarm"].includes(message.type)) return;
    const incoming = message.type === "intrusion_alarm" ? { ...message.data, id: message.data.alert_id, alert_name: "Potential Unauthorized Intrusion", status: "active", alarm_status: "ACTIVE" } : message.data;
...
function Dashboard() {
  ...
  const liveAlerts = message?.data && ["alert", "intrusion_alarm"].includes(message.type) ? [message.data, ...alerts.data.filter((alert) => alert.id !== message.data.id)] : alerts.data;
...
function AlertDetailPage() {
  ...
  useEffect(() => {
    let mounted = true;
    Promise.all([getAlert(alertId), getCameras(), getAlarm(alertId)]).then(([alertData, cameras, alarmData]) => {
      if (!mounted) return;
  ...
  async function changeAlarm(action) {
    setSaving(true);
    try { setAlarm(await (action === "silence" ? silenceAlarm(alertId) : acknowledgeAlarm(alertId))); } catch (requestError) { setError(requestError.message); } finally { setSaving(false); }
  }

  return <Shell><main className="content"><PageHead eyebrow="Incident Response" title="Intrusion Review" sub="Confirm the detection to sound the local operator alarm, or decline it without audio." actions={<button onClick={() => navigate(-1)}>Back</button>} /><State loading={loadedId !== alertId} error={error} empty={alert ? null : "Alert not found"}>{alert && <section className="alert-detail panel"><div className="notification-kicker"><AlertTriangle size={18} /> {decision ? `ALERT ${decision.toUpperCase()}` : "ACTION REQUIRED"}</div><h2>{alert.reason || alert.message || "Intrusion detected"}</h2>{alert.identity_status && <p className="identity-context"><b>Identity:</b> {alert.subject_label || alert.identity_status.toUpperCase()} {alert.face_similarity != null ? ` | ${(Number(alert.face_similarity) * 100).toFixed(1)}% similarity` : ""}</p>}{alert.behavior_type && <p className="behavior-context"><b>{alert.behavior_type.replaceAll("_", " ")}</b> | {alert.behavior_state || "WATCH"} | {alert.behavior_reason || "Temporal behavior signal"}</p>}<div className="alert-detail-grid"><Stat label="CAMERA" value={alert.camera_name || `Camera ${alert.camera_id}`} detail={alert.camera_code || `ID ${alert.camera_id}`} /><Stat label="OBJECT" value={alert.object_type || "Unavailable"} detail={`Track ID ${alert.track_id ?? "Unavailable"}`} /><Stat label="ZONE" value={alert.zone || "Unavailable"} detail={alert.zone_type || "Zone type unavailable"} /><Stat label="THREAT SCORE" value={alert.score ?? "Unavailable"} detail={`Severity ${alert.severity || "Unavailable"}`} /><Stat label="PLATE" value={alert.plate_number || "UNKNOWN"} detail={alert.plate_confidence == null ? "OCR unavailable or low confidence" : `Confidence ${Math.round(alert.plate_confidence * 100)}%`} /></div><div className="alert-detail-meta"><p><b>Detected:</b> {alert.timestamp || alert.created_at || "Unavailable"}</p>{alert.behavior_duration_seconds != null && <p><b>Behavior duration:</b> {Number(alert.behavior_duration_seconds).toFixed(1)} seconds</p>}{alarm?.status && alarm.status !== "INACTIVE" && <p><b>Alarm:</b> {alarm.status}</p>}<p><b>Evidence:</b> {alert.evidence_path ? <a href={`${API_BASE_URL}${alert.evidence_path}`} target="_blank" rel="noreferrer">Open evidence snapshot</a> : "Unavailable"}</p>{alert.watchlist_match && <p className="critical-text"><b>WATCHLIST MATCH:</b> {alert.watchlist_label || alert.plate_number}</p>}</div>{alarm?.status === "ACTIVE" && <div className="decision-actions"><button disabled={saving} onClick={() => changeAlarm("acknowledge")}>Acknowledge Alarm</button><button disabled={saving} onClick={() => changeAlarm("silence")}>Silence Alarm</button></div>}{!decision && <div className="decision-actions"><button className="primary" disabled={saving} onClick={() => decide("confirmed")}>Confirm Intrusion and Sound Alarm</button><button disabled={saving} onClick={() => decide("declined")}>Decline Intrusion</button></div>}{decision && <div className={`decision-result ${decision}`}>{decision === "confirmed" ? "Intrusion confirmed. Local alarm sounded." : "Intrusion declined. No alarm was played."}</div>}{error && <div className="form-error">{error}</div>}</section>}</State></main></Shell>;
}
function Alerts() { const result = useRemote(getAlerts, [], asArray); const [searchParams] = useSearchParams(); const query = (searchParams.get("search") || "").toLowerCase(); const alerts = result.data.filter((alert) => !query || [alert.reason, alert.message, alert.behavior_type, alert.behavior_reason, alert.zone, alert.object_type, alert.vehicle_class, alert.plate_number, alert.camera_id].some((value) => String(value ?? "").toLowerCase().includes(query))); return <Shell><main className="content"><PageHead eyebrow="Incident Response" title="Active Alerts" sub={query ? `Active alerts matching "${query}".` : "Only alerts with active operator review status are shown."} /><State loading={result.loading} error={result.error} reload={result.reload} empty={alerts.length ? null : query ? "No active alerts match this search" : "No active alerts"}><div className="table-wrap"><table><thead><tr><th>ID</th><th>SEVERITY</th><th>EVENT</th><th>BEHAVIOR</th><th>VEHICLE</th><th>PLATE</th><th>SOURCE</th><th>TRACK / OBJECT</th><th>ZONE / SCORE</th><th>TIMESTAMP</th><th>EVIDENCE</th></tr></thead><tbody>{alerts.map((alert) => <tr key={alert.id}><td>{alert.id}</td><td><span className={`severity ${(alert.severity || "unknown").toLowerCase()}`}>{alert.severity || "Unknown"}</span></td><td><strong>{alert.reason || alert.message || "No description"}</strong>{alert.watchlist_match && <small className="critical-text">WATCHLIST MATCH</small>}</td><td>{alert.behavior_type ? <><strong className="behavior-badge">{alert.behavior_type.replaceAll("_", " ")}</strong><small>{alert.behavior_duration_seconds != null ? `${Number(alert.behavior_duration_seconds).toFixed(1)}s` : alert.behavior_state}</small></> : "None"}</td><td>{alert.vehicle_class || "Not classified"}{alert.vehicle_class_confidence != null && <small>{Math.round(alert.vehicle_class_confidence * 100)}%</small>}</td><td>{alert.plate_number || "UNKNOWN"}</td><td>{alert.camera_id ?? "Unavailable"}</td><td>{alert.track_id ?? "Unavailable"} / {alert.object_type || "Unavailable"}</td><td>{alert.zone || "Unavailable"} / {alert.score ?? "-"}</td><td>{alert.timestamp || alert.created_at || "Unavailable"}</td><td>{alert.evidence_path ? <a href={`${API_BASE_URL}${alert.evidence_path}`} target="_blank" rel="noreferrer">Open</a> : "Unavailable"}</td></tr>)}</tbody></table></div></State></main></Shell>; }
...
function Watchlist() {
  const result = useRemote(getWatchlist, [], asArray);
  const [form, setForm] = useState({ plate_number: "", label: "", priority: "HIGH", notes: "" });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState(null);
  async function addEntry(event) {
    event.preventDefault(); setSaving(true); setError(null);
    try { await createWatchlistEntry(form); setForm({ plate_number: "", label: "", priority: "HIGH", notes: "" }); result.reload(); }
    catch (requestError) { setError(requestError.message); } finally { setSaving(false); }
  }
  async function toggle(entry) { try { await updateWatchlistEntry(entry.id, { enabled: !entry.enabled }); result.reload(); } catch (requestError) { setError(requestError.message); } }
  async function rename(entry) { const label = window.prompt("Watchlist label", entry.label); if (!label || label.trim() === entry.label) return; try { await updateWatchlistEntry(entry.id, { label: label.trim() }); result.reload(); } catch (requestError) { setError(requestError.message); } }
  async function remove(entry) { if (!window.confirm(`Delete ${entry.plate_number}?`)) return; try { await deleteWatchlistEntry(entry.id); result.reload(); } catch (requestError) { setError(requestError.message); } }
  return <Shell><main className="content"><PageHead eyebrow="Vehicle Intelligence" title="Plate Watchlist" sub="Manage actual plate observations used by the ANPR threat context." /><form className="camera-form panel" onSubmit={addEntry}><div className="section-title"><div><h2>Add Watchlist Plate</h2><small>Use a synthetic test plate or an explicitly authorized operational entry.</small></div></div><div className="camera-form-grid"><label>Plate Number<input required value={form.plate_number} onChange={(event) => setForm((current) => ({ ...current, plate_number: event.target.value }))} placeholder="GJ01AB1234" /></label><label>Label<input required value={form.label} onChange={(event) => setForm((current) => ({ ...current, label: event.target.value }))} placeholder="Audit vehicle" /></label><label>Priority<select value={form.priority} onChange={(event) => setForm((current) => ({ ...current, priority: event.target.value }))}><option>LOW</option><option>MEDIUM</option><option>HIGH</option><option>CRITICAL</option></select></label><label>Notes<input value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} placeholder="Optional context" /></label></div>{error && <div className="form-error">{error}</div>}<button className="primary" disabled={saving}>{saving ? "Saving..." : "Add Plate"}</button></form><State {...result} empty={result.data.length ? null : "No watchlist plates configured."}><div className="table-wrap"><table><thead><tr><th>PLATE</th><th>LABEL</th><th>PRIORITY</th><th>STATUS</th><th>NOTES</th><th>ACTIONS</th></tr></thead><tbody>{result.data.map((entry) => <tr key={entry.id}><td><strong>{entry.plate_number}</strong></td><td>{entry.label}</td><td>{entry.priority}</td><td><span className={`tag ${entry.enabled ? "blue" : "salmon"}`}>{entry.enabled ? "ENABLED" : "DISABLED"}</span></td><td>{entry.notes || "-"}</td><td><button className="small-btn" onClick={() => rename(entry)}>Edit label</button><button className="small-btn" onClick={() => toggle(entry)}>{entry.enabled ? "Disable" : "Enable"}</button><button className="small-btn" onClick={() => remove(entry)}>Delete</button></td></tr>)}</tbody></table></div></State></main></Shell>;
}

function TrustedPersonForm({ onCancel, onSaved }) { const [label, setLabel] = useState(""); const [category, setCategory] = useState("authorized"); const [image, setImage] = useState(null); const [saving, setSaving] = useState(false); const [error, setError] = useState(null); async function submit(event) { event.preventDefault(); if (!image) { setError("Select one clear reference image."); return; } setSaving(true); setError(null); try { await createTrustedPerson({ label, category, enabled: true, image }); onSaved(); } catch (requestError) { setError(requestError.message); } finally { setSaving(false); } } return <form className="trusted-form panel" onSubmit={submit}><div className="section-title"><div><h2>Add Trusted Person</h2><small>Register one clear synthetic or authorized reference face.</small></div><button type="button" onClick={onCancel}>Cancel</button></div><label>Name / Label<input required value={label} onChange={(event) => setLabel(event.target.value)} placeholder="Authorized Personnel 01" /></label><label>Category<select value={category} onChange={(event) => setCategory(event.target.value)}><option value="authorized">Authorized</option><option value="border_staff">Border Staff</option><option value="visitor">Visitor</option><option value="watchlist">Watchlist</option></select></label><label>Reference Face<input required type="file" accept="image/jpeg,image/png" onChange={(event) => setImage(event.target.files?.[0] || null)} />{image && <small>{image.name}</small>}</label>{error && <div className="form-error">{error}</div>}<div className="page-actions"><button className="primary" disabled={saving}>{saving ? "Registering..." : "Register Trusted Person"}</button></div></form>; }

function TrustedPersons() { const result = useRemote(getTrustedPersons, [], asArray); const [adding, setAdding] = useState(false); const [error, setError] = useState(null); async function toggle(subject) { try { await updateTrustedPerson(subject.id, { enabled: !subject.enabled }); result.reload(); } catch (requestError) { setError(requestError.message); } } async function rename(subject) { const label = window.prompt("Trusted person label", subject.label); if (!label || label.trim() === subject.label) return; try { await updateTrustedPerson(subject.id, { label: label.trim() }); result.reload(); } catch (requestError) { setError(requestError.message); } } async function remove(subject) { if (!window.confirm(`Delete ${subject.label}?`)) return; try { await deleteTrustedPerson(subject.id); result.reload(); } catch (requestError) { setError(requestError.message); } } async function replaceReference(subject, image) { if (!image) return; try { await replaceTrustedPersonReference(subject.id, image); result.reload(); } catch (requestError) { setError(requestError.message); } } function saved() { setAdding(false); result.reload(); } const active = result.data.filter((subject) => subject.enabled).length; return <Shell><main className="content"><PageHead eyebrow="Identity Operations" title="Trusted Persons" sub="Manage local reference identities without exposing biometric embeddings." actions={<button className="primary" onClick={() => { setAdding(true); setError(null); }}>Add Trusted Person</button>} />{adding && <TrustedPersonForm onCancel={() => setAdding(false)} onSaved={saved} />}{error && <div className="state error-state">{error}</div>}<State {...result} empty={result.data.length ? null : "No trusted persons registered."}><div className="stats four"><Stat label="REGISTERED" value={result.data.length} /><Stat label="ACTIVE" value={active} /><Stat label="DISABLED" value={result.data.length - active} /><Stat label="RECOGNITION" value="OPTIONAL" detail="SFace model + runtime setting" /></div><div className="trusted-grid">{result.data.map((subject) => <article className="trusted-card panel" key={subject.id}><div className="section-title"><div><h2>{subject.label}</h2><small>TRUSTED-{String(subject.id).padStart(3, "0")}</small></div><span className={`tag ${subject.enabled ? "blue" : "salmon"}`}>{subject.enabled ? "ACTIVE" : "DISABLED"}</span></div><p>Category: <b>{subject.category.replaceAll("_", " ")}</b></p><small>Registered {subject.created_at}</small><div className="trusted-actions"><button onClick={() => rename(subject)}>Edit label</button><button onClick={() => toggle(subject)}>{subject.enabled ? "Disable" : "Enable"}</button><label className="replace-button">Replace face<input type="file" accept="image/jpeg,image/png" onChange={(event) => replaceReference(subject, event.target.files?.[0])} /></label><button onClick={() => remove(subject)}>Delete</button></div></article>)}</div></State></main></Shell>; }

function Incidents() { const result = useRemote(getIncidents, [], asArray); const [actionError, setActionError] = useState(null); async function changeStatus(incident, status) { setActionError(null); try { await updateIncident(incident.id, status); result.reload(); } catch (error) { setActionError(error.message); } } return <Shell><main className="content"><PageHead eyebrow="Incident Response" title="Incidents" sub="Protected-zone intrusions remain open until handled separately from alarm silencing." /><State {...result} empty={result.data.length ? null : "No incidents recorded"}>{actionError && <div className="state error-state">{actionError}</div>}<div className="table-wrap"><table><thead><tr><th>INCIDENT</th><th>ALERT</th><th>CAMERA</th><th>TITLE</th><th>STATUS</th><th>CREATED</th><th>ACTION</th></tr></thead><tbody>{result.data.map((incident) => <tr key={incident.id}><td>{incident.incident_code}</td><td>{incident.alert_id}</td><td>{incident.camera_id}</td><td>{incident.title}</td><td><span className="tag blue">{incident.status.toUpperCase()}</span></td><td>{incident.created_at}</td><td>{incident.status !== "resolved" && <button className="small-btn" onClick={() => changeStatus(incident, incident.status === "open" ? "acknowledged" : "resolved")}>{incident.status === "open" ? "Acknowledge" : "Resolve"}</button>}</td></tr>)}</tbody></table></div></State></main></Shell>; }

function Events() { const result = useRemote(getEvents, [], asArray); return <Shell><main className="content"><PageHead eyebrow="Historical Intelligence" title="Event Archive" sub="Recorded events returned by the surveillance backend." /><State {...result} empty={result.data.length ? null : "No events recorded"}><div className="table-wrap"><table><thead><tr><th>EVENT ID</th><th>TYPE</th><th>CAMERA</th><th>BEHAVIOR</th><th>VEHICLE</th><th>PLATE</th><th>IDENTITY</th><th>SEVERITY</th><th>DESCRIPTION</th><th>TIMESTAMP</th></tr></thead><tbody>{result.data.map((event) => <tr key={event.id}><td>{event.id}</td><td>{event.event_type || "Unavailable"}</td><td>{event.camera_id ?? "Unavailable"}</td><td>{event.behavior_type ? <><strong className="behavior-badge">{event.behavior_type.replaceAll("_", " ")}</strong><small>{event.duration_seconds != null ? `${Number(event.duration_seconds).toFixed(1)}s` : event.behavior_state}</small></> : "None"}</td><td>{event.vehicle_class || "Not classified"}{event.vehicle_class_confidence != null && <small>{Math.round(event.vehicle_class_confidence * 100)}%</small>}</td><td>{event.plate_number || "UNKNOWN"}{event.watchlist_match && <small className="critical-text">WATCHLIST MATCH</small>}</td><td>{event.subject_label || event.identity_status || "Unavailable"}</td><td>{event.severity || "Unavailable"}</td><td>{event.description || "No description"}</td><td>{event.timestamp || "Unavailable"}</td></tr>)}</tbody></table></div></State></main></Shell>; }
...
function Analytics() { const result = useRemote(getAnalytics, null, asObject); const detections = useRemote(getDetections, [], asArray); const reload = () => { result.reload(); detections.reload(); }; const analytics = result.data || {}; return <Shell><main className="content"><PageHead eyebrow="Operational Intelligence" title="Security Analytics" sub="Metrics calculated from stored detections, alerts, events, and face observations." /><State loading={result.loading || detections.loading} error={result.error || detections.error} reload={reload} empty={result.data ? null : "Analytics data unavailable"}><div className="stats four"><Stat label="TOTAL DETECTIONS" value={analytics.total_detections ?? "Unavailable"} /><Stat label="ACTIVE ALERTS" value={analytics.active_alerts ?? "Unavailable"} tone="critical" /><Stat label="CRITICAL ALERTS" value={analytics.critical_alerts ?? "Unavailable"} /><Stat label="TOTAL EVENTS" value={analytics.total_events ?? "Unavailable"} /><Stat label="FACES DETECTED" value={analytics.faces_detected ?? 0} /><Stat label="KNOWN FACES" value={analytics.known_faces ?? 0} /><Stat label="UNKNOWN FACES" value={analytics.unknown_faces ?? 0} /><Stat label="WATCHLIST MATCHES" value={analytics.face_watchlist_matches ?? 0} tone="critical" /><Stat label="SUSPICIOUS ACTIVITY" value={analytics.suspicious_activities ?? 0} tone="critical" /><Stat label="LOITERING" value={analytics.loitering ?? 0} /><Stat label="FENCE CROSSINGS" value={analytics.repeated_fence_crossings ?? 0} /><Stat label="EXTENDED PRESENCE" value={analytics.extended_restricted_presence ?? 0} /><Stat label="NIGHT MOVEMENTS" value={analytics.night_movements ?? 0} /><Stat label="NIGHT INTRUSIONS" value={analytics.night_intrusions ?? 0} tone="critical" /><Stat label="NIGHT VEHICLES" value={analytics.night_vehicle_movements ?? 0} /></div><div className="chart-grid"><Chart title="Detection Trends" count={detections.data.length} /><Chart title="Alert Distribution" count={analytics.critical_alerts} /><Chart title="Stored Activity" count={analytics.total_events} /></div></State></main></Shell>; }
...
function SettingsPage() {
  ...
  const toggles = [["wildlife_suppression", "Wildlife Suppression"], ["severe_weather_compensation", "Severe Weather Compensation"], ["night_vision_filtering", "Night Vision Artifact Filtering"], ["face_recognition_enabled", "Face Recognition"]];
  return <Shell><main className="content"><PageHead eyebrow="System Administration" title="Configuration" sub="Runtime detection settings apply to newly started processing workers." actions={<button className="primary" disabled={!settings || saving} onClick={save}>{saving ? "Saving..." : "Save Settings"}</button>} />{error && <div className="state error-state">{error}</div>}{!settings ? <div className="state">Loading configuration...</div> : <div className="settings-layout"><div className="settings-nav"><b>Detection Thresholds</b><span>Alert Config</span><span>Restricted Zones</span><span>Notification Settings</span><span>Operator Access</span></div><div><section className="settings-card"><h2>AI Sensitivity Calibration</h2><p>Adjust the minimum confidence passed to the active YOLO pipeline. Changes apply when a camera worker starts again.</p><label>Human Detection Confidence <b>{Math.round(settings.detection_confidence * 100)}%</b><input type="range" min="0.1" max="0.99" step="0.01" value={settings.detection_confidence} onChange={(event) => update("detection_confidence", Number(event.target.value))} /></label><label>Vehicle Identification <b>{Math.round(settings.vehicle_confidence * 100)}%</b><input type="range" min="0.1" max="0.99" step="0.01" value={settings.vehicle_confidence} onChange={(event) => update("vehicle_confidence", Number(event.target.value))} /></label></section><section className="settings-card"><h2>Face Recognition</h2><p>Recognition is optional and requires the configured OpenCV SFace model. A face match never replaces zone or behavior policy.</p><label>Match Similarity Threshold <b>{Number(settings.face_recognition_threshold ?? 0.363).toFixed(3)}</b><input type="range" min="0.1" max="0.99" step="0.001" value={settings.face_recognition_threshold ?? 0.363} onChange={(event) => update("face_recognition_threshold", Number(event.target.value))} /></label><label>Confirmation Frames <b>{settings.face_recognition_confirmation_frames ?? 3}</b><input type="range" min="1" max="10" step="1" value={settings.face_recognition_confirmation_frames ?? 3} onChange={(event) => update("face_recognition_confirmation_frames", Number(event.target.value))} /></label></section><section className="settings-card"><h2>Environmental Filters</h2><p>These runtime switches are stored by the backend. They are available for the current operator configuration while the model-specific filters evolve.</p>{toggles.map(([key, name]) => <div className="toggle-row" key={key}><span><b>{name}</b><small>{settings[key] ? "Enabled" : "Disabled"}</small></span><button type="button" className={`setting-toggle ${settings[key] ? "on" : ""}`} aria-pressed={settings[key]} onClick={() => update(key, !settings[key])}><i /></button></div>)}</section></div></div>}</main></Shell>;
}
function Login() {
  const navigate = useNavigate();
  const location = useLocation();
