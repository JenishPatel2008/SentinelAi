...
import { API_BASE_URL, createCamera, decideAlert, deleteCamera, getAlert, getAlerts, getAnalytics, getCameras, getDetections, getEvents, getStreamStatus, startStream, stopStream, uploadVideo } from "./services/api";
import "./App.css";
...
function useStreamSupervisor(cameras) {
  ...
  useEffect(() => {
    ...
    async function ensureStreams() {
      await Promise.all(cameras.filter((camera) => camera.is_active && camera.stream_url).map(async (camera) => {
...
function LivePreview({ camera, alt }) {
  ...
  const source = `${API_BASE_URL}/api/streams/${camera.id}/mjpeg?attempt=${attempt}`;

  useEffect(() => () => clearTimeout(retryTimer.current), [camera.id]);
...
function CameraTile({ camera }) { const status = camera.is_active && camera.stream_url ? "online" : "offline"; return <div className="camera-tile"><div className="camera-feed"><LivePreview camera={camera} alt={`${camera.camera_code} live preview`} /><div className="feed-top"><b>{camera.camera_code}</b><span className={status === "online" ? "live" : "down"}>{status.toUpperCase()}</span></div><div className="feed-bottom"><span>{camera.sector}</span><time>{camera.name}</time></div></div><div className="tile-meta"><div><b>{camera.name}</b><small>{camera.stream_url || "No stream source configured"}</small></div><span className={status}>{status}</span></div></div>; }
...
function Dashboard() {
  const cameras = useRemote(getCameras, [], asArray);
...
function Cameras() { const result = useRemote(getCameras, [], asArray); const [adding, setAdding] = useState(false); const [actionError, setActionError] = useState(null); async function removeCamera(camera) { if (!window.confirm(`Delete ${camera.camera_code}?`)) return; setActionError(null); try { await deleteCamera(camera.id); result.reload(); } catch (error) { setActionError(error.message); } } function saved() { setAdding(false); result.reload(); } return <Shell><main className="content"><PageHead eyebrow="Network Infrastructure" title="Border Cameras" sub="Manage configured network streams and detection endpoints." actions={<button className="primary" onClick={() => { setAdding(true); setActionError(null); }}>Add Camera</button>} />{adding && <AddCameraForm onCancel={() => setAdding(false)} onSaved={saved} />}{actionError && <div className="state error-state">{actionError}</div>}<State {...result} empty={result.data.length ? null : "No cameras configured. Add a camera source to begin surveillance."}><div className="stats four"><Stat label="TOTAL CAMERAS" value={result.data.length} /><Stat label="ONLINE" value={result.data.filter((camera) => (camera.status || (camera.is_active ? "online" : "offline")) === "online").length} /><Stat label="OFFLINE / DEGRADED" value={result.data.filter((camera) => !camera.is_active || camera.status === "offline").length} tone="critical" /><Stat label="AI ACTIVE NODES" value="Unavailable" detail="No node metric endpoint" /></div><div className="table-wrap"><table><thead><tr><th>CAMERA ID</th><th>NAME / LOCATION</th><th>SECTOR</th><th>STATUS</th><th>SOURCE</th><th>DETECTION</th><th>ACTIONS</th></tr></thead><tbody>{result.data.map((camera) => <tr key={camera.id}><td>{camera.camera_code}</td><td>{camera.name}</td><td>{camera.sector}</td><td>{camera.status || (camera.is_active ? "online" : "offline")}</td><td>{camera.stream_url || "Not configured"}</td><td><span className="tag blue">{camera.is_active ? "ENABLED" : "DISABLED"}</span></td><td><button className="small-btn" onClick={() => removeCamera(camera)}>Delete</button></td></tr>)}</tbody></table></div></State></main></Shell>; }
